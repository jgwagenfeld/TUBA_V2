...
file = tkFileDialog.askopenfilename(parent=root,
                                    initialdir='/home/jangeorg/TUBA/examples/000_Testing/007_TUYAU_3d',
...
RenameSource('Results', new_casermed_2)
keys=new_casermed_2.GetProperty("FieldsTreeInfo")[::2]
#Get all the fields contained in the ResultFile
arr_name_with_dis=[elt.split("/") for elt in keys]
newlist=[]
...
new_casermed_2.AllArrays = newlist       
new_casermed_2.GenerateVectors = 1   
...
new_casermed_3 = MEDReader(FileName=file)

RenameSource('Results', new_casermed_3)
keys=new_casermed_3.GetProperty("FieldsTreeInfo")[::2]
#Get all the fields contained in the ResultFile
arr_name_with_dis=[elt.split("/") for elt in keys]
newlist=[]
...
new_casermed_3.AllArrays = newlist       
new_casermed_3.GenerateVectors = 1   
...
# set active source
SetActiveSource(new_casermed_3)

Deformation_Warp= WarpByVector(Input=new_casermed_3)
...
Deformation_Vectors= Glyph(Input=new_casermed_3,GlyphType='Arrow')
...
#------------------------------------------------------------------------------
SetActiveSource(new_casermed_3)
Force_Vectors = Glyph(Input=new_casermed_3,GlyphType='Arrow')
...
#------------------------------------------------------------------------------ 
SetActiveSource(new_casermed_2)
VonMise = ELNOMesh(Input=new_casermed_2)        
...
ColorBy(VonMise_Display, ('POINTS', 'RESU____SIEQ_ELNO'))

RenameSource('VonMise (ELNO-Field)', VonMise)  

RESU____SIEQ_ELNO_LUT = GetColorTransferFunction('RESU____SIEQ_ELNO')
RESU____SIEQ_ELNO_LUT.VectorMode = 'Component'
RESU____SIEQ_ELNO_LUT_ColorBar = GetScalarBar(RESU____SIEQ_ELNO_LUT, renderView1)
RESU____SIEQ_ELNO_LUT_ColorBar.Title = 'VonMise Stress'
RESU____SIEQ_ELNO_LUT_ColorBar.ComponentTitle = 'Magnitude (MPa // N/mm**2)'

                                                 
VonMise_Display.RescaleTransferFunctionToDataRange(False)                                                     
RESU____SIEQ_ELNO_PWF = GetOpacityTransferFunction('RESU____SIEQ_ELNO')
...
try:
    ...
    Visualization_Geom = LegacyVTKReader(FileNames=['//home/jangeorg/TUBA/examples/000_Testing/007_TUYAU_3d/compound_paravis.vtk'])
